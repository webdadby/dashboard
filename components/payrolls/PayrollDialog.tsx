...
export function PayrollDialog({
  ...
  const form = useForm<PayrollFormValues>({
    ...
    defaultValues: {
      ...
      advance_payment: defaultAdvancePayment,
  ...
  const calculateIncomeTax = (amount: number) => amount * (incomeTaxRate / 100);
  ...
  useEffect(() => {
    const subscription = form.watch((value, { name }) => {
      ...
      const incomeTax = roundToTwoDecimals(calculateIncomeTax(totalAccrued));
  ...
  const onSubmit = async (data: PayrollFormValues) => {
    ...
    try {
      const payrollData = {
        ...
        income_tax: calculatedValues.incomeTax,
        pension_tax: calculatedValues.pensionTax,
